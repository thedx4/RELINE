...
int num_threads = 10, dim = 20, num_negative = 5;
...
void *TrainRELINEThread(void *id)
{
...
void TrainRELINE() {
	...
	clock_t start = clock();
	printf("--------------------------------\n");
	for (a = 0; a < num_threads; a++) pt[a] = boost::thread(TrainRELINEThread, (void *)a);
...
int main(int argc, char **argv) {
	...
	if (argc == 1) {
		...
		printf("\nExamples:\n");
		printf("./RELINE -size 200 -negative 5 -samples 100 -rho 0.025 -threads 20\n\n");
		return 0;
	}
